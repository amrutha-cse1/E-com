// dev-server-setup.js (stub)
// Visual-edit dev server middleware has been removed for this demo.
// This file provides a harmless stub so enabling visual edits won't crash the dev server.
module.exports = function setupDevServer(config) {
  // no-op: return config unchanged
  return config;
};
...
function setupDevServer(config) {
  ...
  // Allow specific preview/example subdomains
      if (origin.match(/^https?:\/\/([a-zA-Z0-9-]+\.)*(example\.com|example\.sh)$/)) {
        return true;
      }
